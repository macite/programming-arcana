...
\begin{table}[h]
  ...
  \begin{tabular}{|c|c|l|}
    ...
    Real Numbers   &   \texttt{( ) + - / *} &    \\
       & & \\
    Text           &   \emph{None}          & You cannot perform mathematical\\
    & & operations on text.\\
    \hline
  \end{tabular}
...
\mynote{
\begin{itemize}
  \item The \texttt{int} type is the main whole number type.
  \item The \texttt{double} type is the main real number type.
  \item C has limited support for text data. In most languages, text is represented using a \texttt{String} type. The C text type is named \texttt{c-string} to indicate this limited support. C includes a \nameref{sec:program-creation-library} to add operations to manipulate \texttt{c-string} values.
