...
% \input{topics/programs-and-compilers/other/TextEditor}
\input{topics/programs-and-compilers/c/c_compiler}
\input{topics/programs-and-compilers/pascal/pascal_compiler}
...
A layer, or level, of abstraction provides a means of managing complexity at a similar level of detail. This means that all aspects of that level are similar, and related to each other. When you are working at a certain level of abstraction, you think in terms of the tools and artefacts that exist within that layer. 
...
That is a good question. Computers are unintelligent, but programs are not. The computer does not \emph{know} what it is doing any more than a bicycle \textit{knows} how to move. It is following a set of steps that was coded into the program being executed. These steps, however, were created by people, software developers. When a computer does something cool, its because a person told it to do that.
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item A \textbf{Text Editor} that offers syntax highlighting for the language you are using. We recommend Visual Studio Code.
    \item The \textbf{Compiler} for the language you are using.
    \item \textbf{Bash} and the \textbf{Terminal} (on Windows only - this will already be available on Linux and Mac).
  \end{enumerate}
